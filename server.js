...
const PYTHON = "C:\\Users\\hanse\\AppData\\Local\\Python\\pythoncore-3.14-64\\python.exe";
...
app.post('/api/generar-video', (req, res) => {
    ...
    const contenidoEscapado = contenido.replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/\n/g, '\\n');
...
main_content = open(r"${carpeta}\\main.py").read()
...
`;
    ...
    res.json({ success: true, message: "⏳ Generando video — aparecerá en YouTube en 3-5 minutos", url: `https://studio.youtube.com/channel/${canal_id}/videos` });

    exec(`"${PYTHON}" "${tempFile}"`, { cwd: carpeta }, (err, stdout) => {
        if (err) console.error(`❌ Error ${config.nombre}:`, err.message);
        else {
            const ok = stdout.split('\n').find(l => l.startsWith('SUCCESS:'));
            if (ok) console.log(`✅ Video publicado ${config.nombre}: https://youtube.com/shorts/${ok.replace('SUCCESS:','').trim()}`);
        }
